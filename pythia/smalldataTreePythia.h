...
// Thu Mar 20 15:55:48 2025 by ROOT version 6.35.01
// from TTree tree/tree
// found on file: /Users/magnussellebjerg/BA/pythia/dirtyData.root
//////////////////////////////////////////////////////////

#ifndef smalldataTreePythia_h
#define smalldataTreePythia_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Float_t         pjet3[4];
   UShort_t        nbsv;
   Float_t         pxbsv[1];   //[nbsv]
   Float_t         pybsv[1];   //[nbsv]
   Float_t         pzbsv[1];   //[nbsv]
   Float_t         lbsv[1];   //[nbsv]
   Float_t         elbsv[1];   //[nbsv]
   Float_t         chbsv[1];   //[nbsv]
   UChar_t         codebsv[1];   //[nbsv]
   UShort_t        npart;
   Float_t         pmomx[500];   //[npart]
   Float_t         pmomy[500];   //[npart]
   Float_t         pmomz[500];   //[npart]
   Float_t         pmass[500];   //[npart]
   Float_t         pflight[500];   //[npart]
   UShort_t        nphot;
   Float_t         pphotx[1];   //[nphot]
   Float_t         pphoty[1];   //[nphot]
   Float_t         pphotz[1];   //[nphot]
   UChar_t         pbits[500];   //[npart]
   Int_t           ptype[500];   //[npart]
   UChar_t         ptruth[500];   //[npart]
   UShort_t        pmoth[500];   //[npart]
   ...
   smalldataTreePythia(TTree *tree=0);
   virtual ~smalldataTreePythia();
...
#ifdef smalldataTreePythia_cxx
smalldataTreePythia::smalldataTreePythia(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/Users/magnussellebjerg/BA/pythia/dirtyData.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/Users/magnussellebjerg/BA/pythia/dirtyData.root");
      }
...
smalldataTreePythia::~smalldataTreePythia()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t smalldataTreePythia::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t smalldataTreePythia::LoadTree(Long64_t entry)
{
...
void smalldataTreePythia::Init(TTree *tree)
{
...
bool smalldataTreePythia::Notify()
{
...
void smalldataTreePythia::Show(Long64_t entry)
{
...
Int_t smalldataTreePythia::Cut(Long64_t entry)
{
...
#endif // #ifdef smalldataTreePythia_cxx
